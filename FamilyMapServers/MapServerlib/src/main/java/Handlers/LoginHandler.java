...
// From now on, the right way of moving from bytes to utf-8 characters:
                ...
                isr.close();
                Gson gson = new Gson();
                LoginRequest loginRequest = gson.fromJson(buf.toString(), LoginRequest.class);

                LoginService loginService = new LoginService();

                RegisterLoginResponse response = new RegisterLoginResponse();

                if(loginRequest.validLoginInput() == true) {

                    response = loginService.Post(loginRequest);


                    if (response == null) {

                        purposeMessage.setMessage("Failed to Login User");
                    ...
                    purposeMessage.setMessage("Failed to login User because the " +
                            "input was invalid.");
